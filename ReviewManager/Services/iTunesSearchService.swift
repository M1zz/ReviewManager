...
//  ReviewManager
//
//  iTunes Search API를 통한 앱 아이콘 조회
//
...
actor iTunesSearchService {
    ...
    /// Bundle ID로 앱 아이콘 URL 가져오기 (로컬 캐싱, UI에서 사용)
    func fetchAppIcon(bundleID: String) async throws -> String? {
        ...
        do {
            ...
            guard let iconURLString = try await fetchRemoteIconURL(bundleID: bundleID),
    ...
    /// iTunes API에서 원본 아이콘 URL만 가져오기 (CloudKit 저장용)
    func fetchRemoteIconURL(bundleID: String) async throws -> String? {
        ...
        guard let app = searchResult.results.first else {
            print("⚠️ [iTunesSearch] iTunes에서 앱을 찾을 수 없음: \(bundleID)")
            return nil
        }
